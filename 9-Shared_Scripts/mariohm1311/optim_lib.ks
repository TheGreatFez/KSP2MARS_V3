...
declare function grad_descent {
	...
	until stop_condition = true {
		...
		set step_grad[0] to -settings[2]*(settings[3]*old_grad[0]+grad[0]).
		set step_grad[1] to -settings[2]*(settings[3]*old_grad[1]+grad[1]).
		set step_grad[2] to -settings[2]*(settings[3]*old_grad[2]+grad[2]).
